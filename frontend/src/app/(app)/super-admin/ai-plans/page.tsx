...
import { useAiPlans, useCreateAiPlan, useUpdateAiPlan, useDeleteAiPlan } from '@/features/ai-plans/hooks/useAiPlans';
import { usePlatformAiFeatures } from '@/features/platform-ai-features/hooks/usePlatformAiFeatures';
import { useState } from 'react';

const TIERS = ['ESSENTIALS', 'ADVANCED', 'PRO'];
...
const EMPTY_FORM: AiPlanForm = {
  name: '', tier: 'ESSENTIALS', pricePerMonth: '',
  description: '', isPopular: false, isActive: true, featureKeys: [],
};

// ── Feature checkbox picker ───────────────────────────────────────────────────
function AiFeaturePicker({
  allFeatures,
  selected,
  onChange,
}: {
  ...
  if (allFeatures.length === 0) {
    return (
      <Typography variant="caption" color="text.secondary">
        No active AI features in catalog. Add some via AI Features.
      </Typography>
    );
  }
...
function AiPlanDialog({
  ...
  saving: boolean;
  allFeatures: any[];
  onChange: (k: keyof AiPlanForm, v: any) => void;
      ...
      <DialogContent>
        <Stack gap={2} mt={1}>
          <Stack direction="row" gap={2}>
            ...
            />
            <TextField
              select label="Tier *" value={form.tier} sx={{ minWidth: 160 }}
              disabled={tierLocked}
          ...
          <TextField
            ...
            helperText="AI is priced per org, not per seat. Seeded tiers ship at $0 — set real pricing here."
          />
...
export default function AiPlansPage() {
  const { data: plans = [], isLoading } = useAiPlans(true);
  const { data: allFeatures = [] } = usePlatformAiFeatures(false); // active features only for picker
  const createPlan = useCreateAiPlan();
  const updatePlan = useUpdateAiPlan();
  const deletePlan = useDeleteAiPlan();

  const [createOpen, setCreateOpen] = useState(false);
  const [createForm, setCreateForm] = useState<AiPlanForm>(EMPTY_FORM);

  const [editOpen, setEditOpen] = useState(false);
  const [editId, setEditId] = useState('');
  const [editForm, setEditForm] = useState<AiPlanForm>(EMPTY_FORM);

  const [deleteId, setDeleteId] = useState('');
  const [deleteName, setDeleteName] = useState('');

  const setCreate = (k: keyof AiPlanForm, v: any) => setCreateForm((f) => ({ ...f, [k]: v }));
  const setEdit = (k: keyof AiPlanForm, v: any) => setEditForm((f) => ({ ...f, [k]: v }));
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="Tiered AI bundles sold independently of the core construction plan."
        actions={
      ...
      {(plans as any[]).length === 0 ? (
        <Typography color="text.secondary" sx={{ mt: 4, textAlign: 'center' }}>
          No AI plans yet. Create the first AI plan above.
        </Typography>
      ) : (
        <Box sx={{
            ...
            return (
              <Card key={plan._id} elevation={0} sx={{
                ...
                <CardContent sx={{ p: 3, pt: plan.isPopular || plan.isActive === false ? 5 : 3 }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    {resolvedFeatures.length} AI capability{resolvedFeatures.length === 1 ? '' : 'ies'}
                  </Typography>
      ...
      {/* ── Create ── */}
      <AiPlanDialog
        open={createOpen} title="New AI plan"
      ...
      {/* ── Edit ── */}
      <AiPlanDialog
        open={editOpen} title="Edit AI plan"
      ...
      <Dialog open={!!deleteId} onClose={() => setDeleteId('')} maxWidth="xs" fullWidth>
        <DialogTitle>Delete AI plan</DialogTitle>
        <DialogContent>
          <Typography>
            Are you sure you want to permanently delete <strong>{deleteName}</strong>?
            Any org subscribed to this AI plan will lose AI access.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
