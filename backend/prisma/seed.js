...
async function main() {
    ...
    const managerPerms = permissions.filter((p) => ['read:projects', 'write:projects', 'read:tasks', 'write:tasks', 'read:reports', 'write:reports', 'read:budget', 'read:documents', 'write:documents', 'read:users'].includes(p.name));
    await Promise.all(managerPerms.map((p) => prisma.rolePermission.upsert({
        where: { roleId_permissionId: { roleId: managerRole.id, permissionId: p.id } },
        update: {},
        create: { roleId: managerRole.id, permissionId: p.id },
    })));
    const engineerPerms = permissions.filter((p) => ['read:projects', 'read:tasks', 'write:tasks', 'read:reports', 'write:reports', 'read:documents'].includes(p.name));
    await Promise.all(engineerPerms.map((p) => prisma.rolePermission.upsert({
